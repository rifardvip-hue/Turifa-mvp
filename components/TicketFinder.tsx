...
export default function TicketFinder({ raffleId }: { raffleId: string }) {
  ...
  const [hasOrders, setHasOrders] = useState(false); // ⬅️ nuevo
  ...
  async function onSearch(e?: React.FormEvent) {
    ...
    const params = new URLSearchParams({ raffle: raffleId });

    if (isFourDigits(raw)) {
      setMode("digits");
      params.set("digits", raw);
    } else if (isEmailCom(raw)) {
      setMode("contact");
      params.set("email", raw);
      params.set("correo", raw); // tolerancia
    } else {
      ...
      params.set("telefono", phone); // tolerancia
    }

    const url = `/api/tickets/find?${params.toString()}`;
    ...
    try {
      ...
      setHasOrders(Boolean(json.has_orders));
  ...
  return (
    <div className="bg-gray-900/40 border border-gray-800 rounded-2xl p-4 sm:p-5 mt-4">
      ...
      {open && (
        ...
        >
          <button aria-label="Cerrar" onClick={() => setOpen(false)} className="absolute inset-0 bg-black/70" />
          <div className="relative w-full sm:max-w-2xl sm:rounded-2xl sm:overflow-hidden bg-gradient-to-br from-gray-900 to-black border border-gray-700 shadow-2xl">
            <div className="flex items-center justify-between p-4 border-b border-gray-700">
              ...
              <button onClick={() => setOpen(false)} className="w-9 h-9 rounded-lg bg-gray-800 hover:bg-gray-700 text-white" title="Cerrar">✕</button>
            </div>
            ...
            <div className="p-4 max-h-[70vh] overflow-y-auto">
              ...
              {/* Mostrar números si el backend lo permite (da igual el modo) */}
              {!error && revealsDigits && rows && (
                <div className="bg-gray-800/60 rounded-2xl border border-gray-700">
                  {rows.length === 0 ? (
                    <div className="px-4 py-6 text-gray-300">No hay boletos para los criterios buscados.</div>
                  ) : (
              ...
              {/* Si NO se revelan dígitos (p.ej. no confirmados o sin órdenes) */}
              {!error && !revealsDigits && mode === "contact" && (
                <div className="text-sm text-gray-300 bg-gray-800/50 border border-gray-700 rounded-xl px-4 py-3">
                  {hasOrders
                    ...
                    : "No existe ninguna compra registrada para ese contacto."}
                </div>
              )}

              {!error && mode === "digits" && !revealsDigits && (
                <div className="text-sm text-gray-300 bg-gray-800/50 border border-gray-700 rounded-xl px-4 py-3">
                  No hay boletos con esos 4 dígitos.
                </div>
              )}
            </div>

            <div className="p-4 border-t border-gray-700 flex justify-end">
              <button onClick={() => setOpen(false)} className="px-4 py-2 rounded-lg font-semibold bg-gray-700 text-white hover:bg-gray-600">
                Cerrar
              </button>
            </div>
          </div>
